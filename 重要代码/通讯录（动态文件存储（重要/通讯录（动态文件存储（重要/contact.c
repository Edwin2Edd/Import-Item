...
void CheckContactCap(Contact* pc);
...
void LoadContact(Contact* pc) {
	...
	PeoInfo cmp = { 0 };
	while (fread(&cmp, sizeof(PeoInfo), 1, pf))
	{
		CheckContactCap(pc);
		pc->data[pc->sz] = cmp;
		pc->sz++;
...
void CheckContactCap(Contact* pc) {//检测空间
	assert(pc);
...
void DataOutContact(Contact* pc) {
